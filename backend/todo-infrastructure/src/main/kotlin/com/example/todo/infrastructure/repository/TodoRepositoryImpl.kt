...
class TodoRepositoryImpl(
    ...
    override fun findByUserAndCompleted(user: User, completed: Boolean): List<Todo> {
        val userEntity = user.id?.let { userJpaRepository.findById(it).orElse(null) }
            ?: return emptyList()
    ...
    override fun findByUserAndTitleContaining(user: User, keyword: String): List<Todo> {
        val userEntity = user.id?.let { userJpaRepository.findById(it).orElse(null) }
            ?: return emptyList()
        return todoMapper.toDomainList(
    ...
    override fun findOverdueTodos(user: User, date: LocalDate): List<Todo> {
        val userEntity = user.id?.let { userJpaRepository.findById(it).orElse(null) }
            ?: return emptyList()
        return todoMapper.toDomainList(
    ...
    override fun findTodosByDueDate(user: User, date: LocalDate): List<Todo> {
        val userEntity = user.id?.let { userJpaRepository.findById(it).orElse(null) }
            ?: return emptyList()
        return todoMapper.toDomainList(
